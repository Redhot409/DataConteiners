...
template<typename T> class ForwardList
{
	...
	public:
		...
		Element(T Data, Element* pNext = nullptr) : Data(Data), pNext(pNext) // Присвоили nullprt, потому что так проще задать в конструкторе по умолчанию
	...
	class ForwardList
	{
		...
		T size; // Переменная, которая в конструкторе по умолчанию обуняется
	public:
		Iterator begin()
		{
			return Head;
		...
		explicit ForwardList(T size) :ForwardList()// Конструктор на заданное кол-во элементов списка
		{
			while (size--)push_front(0);
		}

		ForwardList(const ForwardList<T>& other) :ForwardList() // конструктор копирования. Он должен перебрать все элементы и записать в новый список
		{
	...
	// 3 - Голову перенаправляем на новый элемент.Указатель на Адрес нового элемента кладем в голову
		...
		T& operator [](T index)// Если возвращать просто по значению int operator(без амперсанда &), то
		...
		const T& operator [](T index)const// Если возвращать просто по значению int operator(без амперсанда &), то
			// у нас возвращается временно безымянный объект, который априори является константой
			// поэтому, возникает ошибка list[i]=rand() % 100; в вызове (265) - меняем как бы конст объект
		{
			Element* Temp = Head;
		...
		void pop_front()
		{
			...
			ForwardList::Element* Erased = Head;// Эта запись и означает: Создать указатель на элемент Erased и присвоить ему указатель Head 
		...
		void pop_back()
		{
			...
			ForwardList::Element* Temp = Head;// и для этого Создаем итератор Temp и присваиваем ему тот же указатель, что и у Head
